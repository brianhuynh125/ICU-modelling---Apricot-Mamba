...
outcomes_uf = outcomes_uf[
    ["icustay_id", "mv", "pressor", "crrt", "transfusion", "final_state", "shift_id", "interval"]
]

outcomes_uf = outcomes_uf.rename({"pressor": "vp"}, axis=1)
outcomes_uf = outcomes_uf.rename({"transfusion": "bt"}, axis=1)
...
outcomes.loc[(outcomes["bt"] == 1), "bt"] = "bt"
outcomes.loc[(outcomes["bt"] == 0), "bt"] = "no bt"
...
def transitions(group):
    ...
    group[f"transition_bt"] = group["bt"].shift(1) + "-" + group["bt"]
    return group
