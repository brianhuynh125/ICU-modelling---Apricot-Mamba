...
admissions = pd.read_csv("%s/final_data/admissions_mimic.csv" % OUTPUT_DIR)
...
bmi = pd.concat([bmi, pd.read_csv("%s/mimic/height_weight_conv.csv" % DATA_DIR)])
...
admissions = pd.read_csv(
    "%s/mimic/admissions.csv" % DATA_DIR, usecols=["subject_id", "race"]
).drop_duplicates(subset=["subject_id"])

patients = patients.merge(admissions, on=["subject_id"], how="inner")

admissions = pd.read_csv(
    "%s/mimic/icustays.csv" % DATA_DIR, usecols=["subject_id", "stay_id"]
)
...
cols = ["stay_id", "gender", "anchor_age", "race", "bmi"] + [
    comob for comob in static_eicu.iloc[:, 5:].columns.tolist()
]
...
static.to_csv("%s/final_data/static.csv" % OUTPUT_DIR, index=None)

if not os.path.exists(f"{OUTPUT_DIR}/model"):
    os.makedirs(f"{OUTPUT_DIR}/model")

pickle.dump(scalers, open("%s/model/scalers_static.pkl" % OUTPUT_DIR, "wb"))
